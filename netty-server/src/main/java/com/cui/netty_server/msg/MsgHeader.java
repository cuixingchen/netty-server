...
import com.cui.netty_server.nettyutil.msg.IMsgHead;
 ...
 * 
 */
public class MsgHeader implements IMsgHead {
	...
	public byte[] tobytes() {
		...
		// String[] sversion=version_flag.split(".");
		// byte[] bb=new byte[3];
		// bb[2]=Byte.parseByte(sversion[2]);
		// bb[1]=Byte.parseByte(sversion[1]);
		// bb[0]=Byte.parseByte(sversion[0], 1);
		// b.put(bb);
		b.put(version_flag);
		b.put(encrypt_flag);
	...
	public boolean frombytes(byte[] b) {
		try {
			...
			// this.version_flag=System.arraycopy(b, srcPos, dest, destPos,
			// length);
			// "V"+bf.get(offset)+"."+bf.get(offset+1)+"."+bf.get(offset+2);
			offset += 3;
